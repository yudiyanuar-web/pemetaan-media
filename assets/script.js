...
let STATE = { regions: [], mediaTypes: [], active: null, chart: null, provinceChart: null };
...
function selectRegion(id){
  ...
  document.getElementById("detail-signal").textContent = `Kekuatan sinyal: ${region.signal}`;
...
async function mockAiSuggestion(region, topGaps){
  ...
  const templates = {
    "Video Pembelajaran": { format: "Seri video pendek 3–5 menit", description: "Video per sub-topik dengan animasi sederhana, bisa diunduh untuk daerah bersinyal lemah." },
    "Modul Interaktif / E-learning": { format: "Modul web interaktif ringan", description: "Modul dengan kuis dan umpan balik langsung, dioptimalkan untuk koneksi lambat." },
    "Media Cetak": { format: "Lembar kerja & poster cetak", description: "Materi cetak sebagai cadangan untuk wilayah dengan akses internet terbatas." },
    "Podcast / Audio Pembelajaran": { format: "Podcast audio 10–15 menit", description: "Cocok untuk daerah dengan sinyal terbatas karena ukuran file kecil." },
...
function renderAiResult(ideas, isDemo){
  ...
  const demoNote = isDemo
    ...
    : "";
  box.innerHTML = demoNote + ideas.map(idea => `
    <div class="ai-idea">
