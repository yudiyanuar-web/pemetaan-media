...
2. Minta Claude Code (atau Claude di claude.ai) membuatkan game/media serupa untuk topik itu —
   berikan konteks: mapel dan jenjang yang relevan untuk wilayah tersebut
...
## Hasil AI disimpan di repo (bukan cuma muncul sesaat di browser)

Ini yang membedakan setup ini dari sekadar "chatbot di website": begitu AI selesai membuat saran
untuk satu wilayah, hasilnya **disimpan sebagai file** — `data/ai-suggestions.json` — dan
ditautkan lewat git. Efeknya:

- **Siapa pun yang buka dashboard langsung baca hasil yang sama**, tanpa perlu API key sendiri
  dan tanpa dashboard perlu memanggil AI lagi
- Perubahannya **tercatat di histori git** (`git log data/ai-suggestions.json`) — kelihatan kapan
  dan apa yang berubah tiap kali di-regenerate
- Kalau backend AI (`ai-worker/`) mati atau belum di-setup, dashboard tetap menampilkan saran
  yang sudah tersimpan — fitur inti tidak bergantung pada AI live

**Isi `data/ai-suggestions.json` yang ada sekarang** adalah versi awal untuk 27 wilayah, disusun
berdasarkan pola yang sama seperti prompt di `ai-worker/worker.js` (mempertimbangkan jenis media
dengan gap terbesar tiap wilayah), sebagai titik awal yang siap dipakai.
**Sebelum dipublikasikan resmi**, sebaiknya generate ulang lewat `scripts/generate-ai-suggestions.js`
memakai API key kamu sendiri, supaya hasilnya benar-benar dari pemanggilan Claude API real-time
dan bisa kamu review satu per satu.

### Alur kerja yang disarankan

1. Update `data/media-mapping.json` dengan data kebutuhan/penggunaan media yang sebenarnya
2. Jalankan `node scripts/generate-ai-suggestions.js` (butuh `ANTHROPIC_API_KEY`) — ini memanggil
   Claude API sekali per wilayah, lalu menulis `data/ai-suggestions.json`
3. **Review isinya** — baca tiap saran, buang/edit yang kurang pas (AI tetap perlu dicek manusia)
4. `git commit` file itu ke repo
5. Dashboard otomatis menampilkannya ke semua pengunjung, tanpa panggilan AI tambahan

Tombol **"Buat ulang dengan AI"** di dashboard tetap ada untuk generate versi baru secara live
(butuh backend `ai-worker/` aktif) — tapi itu sifatnya pratinjau di browser, belum otomatis
tersimpan ke repo sampai kamu jalankan script di atas dan commit ulang.
...
2. **"Ide kreatif dari AI"** (tombol di bawahnya) — memakai Claude API sungguhan untuk mengusulkan
   *bentuk konkret* dari media itu (misalnya "game edukasi HTML5 ringan" vs "video 5 menit" vs
   "podcast singkat untuk pengulangan materi"), sesuai jenis media yang gap-nya paling besar.
